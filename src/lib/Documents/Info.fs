...
module Info =
    ...
    /// </summary>
    let query<'a> (props: DbProperties.T) (name: string) (id: obj) : Async<Result<'a>> =
        async {
            if System.String.IsNullOrWhiteSpace(name) then
                ...
                let request = createGet props (sprintf "%s/%s" name (id |> string)) []
                let! result = sendRequest request
                let iResult = result :> IRequestResult
                let jsonDeserialize = fun result -> match result with
                                                    | SuccessResult result ->
                                                        // TODO: extract the _XYZ fields from the object root
                                                        try
